...
//
// Implementation of part 3a - initial 3D portal rendering
//
// Joseph21, august 13, 2022
//
// Dependencies:
...
/* Short description
   -----------------
   This implementation is the follow up of implementation part 2b. See the description of that implementation as well (and check on
   ...
   This part implements the most basic form of 3D portal rendering. The portals are not yet rendered themselves, but are filled with a
   red placeholder (for now).
   The implementation is done in additional convenience function Intersect(), vline() for drawing vertical line segments. The bulk
   of the algorithm is implemented in DrawScreen(), which is called from the render loop in OnUserUpdate().
...
private:
    ...
    void DrawScreen() {
        ...
        item now = { player.sector, 0, WIDTH - 1 };
        ...
        for (int s = 0; s < nrVertices; s++) {
            ...
            for (int x = beginx; x <= endx; x++) {
                ...
                vline( x, ytop[x], cya - 1, olc::VERY_DARK_GREY, olc::GREY, olc::VERY_DARK_GREY );
                ...
                if (neighbor >= 0) {
                    // Placeholder only
                    vline( x, cya, cyb, olc::VERY_DARK_RED, olc::RED, olc::VERY_DARK_RED );
                } else {
...
public:
	...
	bool OnUserUpdate(float fElapsedTime) override {
        ...
        // keep shift pressed for speed up factor
        float fSpeedup = GetKey( olc::Key::SHIFT ).bHeld ? 5.0f : 2.0f;
        // rotate
