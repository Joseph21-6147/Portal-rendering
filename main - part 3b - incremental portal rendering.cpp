...
//
// Implementation of part 3b - incremental 3D portal rendering
//
// Joseph21, august 14, 2022
//
// Dependencies:
...
/* Short description
   -----------------
   This implementation is the follow up of implementation part 3a. See the description of that implementation as well (and check on
   ...
   This part implements the incremental nature of 3D portal rendering: with each portal's rendering there's a possibility that an adjacent (neighbouring)
   portal is encountered. The previous implementation just rendered these with a red placeholder. In this implementation DrawScreen() is working
   with a queue of portals still to process. This queue is processed in a loop that terminates if the queue is empty. If a neighbouring portal is
   encountered, it is put in the queue for future processing. The queue is built using a std::deque.
   Additionally, in OnUserUpdate() some functionality is added to speed up or slow down the movement at will.
