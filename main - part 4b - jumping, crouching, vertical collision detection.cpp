...
//
// Implementation of part 4b - jumping, crouching, vertical collision detection
//
...
/* Short description
   -----------------
   This implementation is the follow up of implementation part 4a. See the description of that part as well (and check on the differences
   between that cpp file and this one).

   After studying and implementing Bisqwits code, I decided there could be a more comprehensive and modular
   build up of the code. With this progressive series of implementations (where each part builds upon the
   previous part) I take a shot at it.

   The core of portal rendering was implemented in part 3b. The parts after that are fancying up that code with various concepts.
   This part implements some vertical movement: jumping and crouching, including the associated vertical collision detection to keep
   the player between the ceiling and the floor. Apart from an additional constant and some additional state variables, the bulk of
   the changes is done in OnUserUpdate() in the user interaction part.
...
#define DuckHeight   2.5f              // ... and when crouching (ducking)
...
private:
    ...
    void DrawScreen() {
        ...
        // Not sure why these pieces of code are inserted - it must be to prevent (or break) some kind of race condition
        std::vector<int> renderedsectors;
        renderedsectors.reserve( sectors.size() );
        for (int n = 0; n < (int)sectors.size(); n++) {
            renderedsectors[n] = 0;
        }
        ...
        do {
            ...
            for (int s = 0; s < nrVertices; s++) {
                ...
                // little lambda to implement looking up and down
                auto Yaw = [=]( float y, float z ) -> float {
                ...
                for (int x = beginx; x <= endx; x++) {
                    ...
                    vline( x, ytop[x], cya - 1, olc::DARK_GREY, olc::VERY_DARK_GREY, olc::DARK_GREY );
                    // Render floor: everything below this sector's floor height
                    vline( x, cyb + 1, ybottom[x], olc::BLUE, olc::DARK_BLUE, olc::BLUE );
                    ...
                    if (neighbor >= 0) {
                        ...
                        vline( x, cya, cnya - 1, olc::BLACK, ((x == x1 || x == x2) ? olc::BLACK : olc::GREY), olc::BLACK );     // Between our and their ceiling
                        ytop[x] = std::clamp( std::max( cya, cnya ), ytop[x], HEIGHT - 1 );         // Shrink remaining window below these ceilings

                        // If our floor is lower than their floor, render bottom wall
                        vline( x, cnyb + 1, cyb, olc::BLACK, ((x == x1 || x == x2) ? 0 : olc::Pixel( 191, 64, 191 )), olc::BLACK );     // Between their and our floor
                        ybottom[x] = std::clamp( std::min( cyb, cnyb ), 0, ybottom[x] );            // Shrink remaining window above these floors
                    } else {
                        // There's no neighbor. Render wall from top (cya = ceiling level) to bottom (cyb = floor level)
                        vline( x, cya, cyb, olc::BLACK, ((x == x1 || x == x2) ? olc::BLACK : olc::GREY), olc::BLACK );
                    }
                }
    ...
    int moving = 0, ground = 0, falling = 1, ducking = 0;
    int prevMouseX, prevMouseY;   // to cache mouse positions for future frame
    float yaw = 0.0f;
...
public:
	...
	bool OnUserUpdate(float fElapsedTime) override {
        ...
        ground = !falling;
        if (falling) {
            player.velocity.z -= 4.0f * fElapsedTime;    // add gravity
        ...
        // Mouse aiming
        int curMouseX = GetMouseX(), curMouseY = GetMouseY();
        int relMouseX = curMouseX - prevMouseX; prevMouseX = curMouseX;
        int relMouseY = curMouseY - prevMouseY; prevMouseY = curMouseY;

        player.angle += relMouseX * 1.5f * fElapsedTime;
        yaw = std::clamp( yaw + relMouseY * 5.0f * fElapsedTime, -5.0f, 5.0f );
        ...
        // jump
        if (GetKey( olc::Key::SPACE  ).bPressed) { if (ground) { player.velocity.z += 0.8f; falling = 1; } }
        // duck
        ducking = 0;
