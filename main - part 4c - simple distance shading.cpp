...
//
// Implementation of part 4c - simple distance shading
//
...
/* Short description
   ...
   This part implements a simple form of distance shading with only a few additional lines of code. The effect on the rendering is impressive!
            ...
            for (int s = 0; s < nrVertices; s++) {
                ...
                for (int x = beginx; x <= endx; x++) {
                    ...
                    if (neighbor >= 0) {
                        ...
                        olc::Pixel col1 = olc::Pixel( 1, 1, 1 ) * (255 - z    ),      // shaded colour for top panel
                                   col2 = olc::Pixel( 6, 2, 6 ) * ( 31 - z / 8);      // shaded colour for bottom panel
                        vline( x, cya, cnya - 1, olc::BLACK, ((x == x1 || x == x2) ? olc::BLACK : col1), olc::BLACK );     // Between our and their ceiling
                        ytop[x] = std::clamp( std::max( cya, cnya ), ytop[x], HEIGHT - 1 );         // Shrink remaining window below these ceilings

                        // If our floor is lower than their floor, render bottom wall
                        vline( x, cnyb + 1, cyb, olc::BLACK, ((x == x1 || x == x2) ? 0 : col2), olc::BLACK );     // Between their and our floor
                        ybottom[x] = std::clamp( std::min( cyb, cnyb ), 0, ybottom[x] );            // Shrink remaining window above these floors
                    } else {
                        // There's no neighbor. Render wall from top (cya = ceiling level) to bottom (cyb = floor level)
                        olc::Pixel col = olc::Pixel( 1, 1, 1 ) * (255 - z);      // shaded colour for wall panel
                        vline( x, cya, cyb, olc::BLACK, ((x == x1 || x == x2) ? olc::BLACK : col), olc::BLACK );
                    }
                }
...
public:
	...
	bool OnUserUpdate(float fElapsedTime) override {
        ...
        yaw = std::clamp( yaw + relMouseY * 7.5f * fElapsedTime, -5.0f, 5.0f );
